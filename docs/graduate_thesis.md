...
| Vm0 | 2.5-isfFactor×0.2 | 2.0 | 2.5 | 2.8 | 论文值 (mg/kg/min) |
| VmX | 0.05-isfFactor×0.01 | 0.04 | 0.05 | 0.07 | 论文值 (mg/kg/min per pmU/L) |
| hepaticBase | 2.07+isfFactor×0.4 | 2.87 | 2.07 | 1.47 | 重平衡稳态Gb=7.0 |
| kp3 | 0.045-isfFactor×0.007 | 0.028 | 0.038 | 0.047 | 抵抗→胰岛素作用更慢 |

> ★ v3.0.20 修复: App默认参数 Vm0=4.25 比 DallaMan 2007 论文值 2.5 高 70%，即使无输入血糖也持续下降到约 6 (vs 期望 Gb=7.0 稳态)。修正: Vm0 4.5→2.5, Km0 25→100, VmX 0.16→0.05, k1 0.040→0.060, hepaticBase 1.8→2.07。4 场景 240min Python 验证通过。
...
（1）把TCN数据驱动模型和Dalla Man七隔室生理模型通过BMA动态融合。OhioT1DM和HUPA两个公开数据集上的30分钟预测MAE 0.612和0.631 mmol/L，Clarke A区92.4%和90.8%。生理模型在数据稀疏时提供安全约束。
