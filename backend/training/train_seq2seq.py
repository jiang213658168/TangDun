# backend/training/train_seq2seq.py
# 序列到序列模型 - 预测未来24个点（每5分钟一个）
#
# 关键改进:
# 1. 输出24个点（完整曲线）
# 2. 平滑损失函数（惩罚锯齿）
    ...
    def __init__(self, data_path, patient_ids, max_samples=2000, horizon_steps=24):
        ...
        for idx, pid in enumerate(patient_ids):
            ...
            for i in indices:
                ...
                # 目标：未来24个点的血糖值
                target = pdata[i:i+horizon_steps]
...
class Seq2SeqModel(nn.Module):
    ...
    def __init__(self, input_dim=10, hidden_dim=128, output_steps=24):
        super().__init__()
...
class SmoothCurveLoss(nn.Module):
    """平滑曲线损失函数"""
    ...
    def forward(self, pred, actual):
        ...
        actual_d2 = actual[:, 2:] - 2 * actual[:, 1:-1] + actual[:, :-2]
        smooth_loss = self.mse(pred_d2, actual_d2)

        # 形状损失（一阶差分）
        pred_d1 = pred[:, 1:] - pred[:, :-1]
        actual_d1 = actual[:, 1:] - actual[:, :-1]
        shape_loss = self.mse(pred_d1, actual_d1)

        return self.alpha * mse_loss + self.beta * smooth_loss + self.gamma * shape_loss
...
def evaluate(model, loader, device):
    ...
    # 计算曲线平滑度（二阶差分的标准差）
    smoothness = np.mean([np.std(np.diff(np.diff(p))) for p in preds])
...
def main():
    ...
    # 加载数据
    print("\n加载训练集...")
    train_dataset = Seq2SeqDataset(data_path, train_pids)
    print("\n加载验证集...")
    val_dataset = Seq2SeqDataset(data_path, val_pids)
    ...
    for epoch in range(100):
        ...
        print(f"Epoch {epoch+1:3d}/100 ({time.time()-t0:.0f}s) | "
              ...
              f"Clarke A(5min): {metrics['clarke_a_5min']:.1f}% | "
              f"平滑度: {metrics['smoothness']:.3f}{status}")
        ...
        if metrics['mae'] < best_mae:
            ...
            torch.save({
                ...
                'hidden_dim': 128,
                'output_steps': 24,
            }, os.path.join(data_dir, 'model_seq2seq.pt'))

    # 保存历史
    with open(os.path.join(data_dir, 'training_history_seq2seq.json'), 'w') as f:
    ...
    print("加载测试集...")
    print("=" * 60)
    test_dataset = Seq2SeqDataset(data_path, test_pids)
    ...
    print(f"整体MAE: {test_metrics['mae']:.3f} mmol/L")
    print(f"5分钟Clarke A: {test_metrics['clarke_a_5min']:.1f}%")
    print(f"曲线平滑度: {test_metrics['smoothness']:.3f}")
    print(f"\n各时间步MAE:")
    ...
    results = {
        ...
        'smoothness': float(test_metrics['smoothness']),
        'output_steps': 24,
        'model': 'Seq2Seq with Attention',
    }
    with open(os.path.join(data_dir, 'results_seq2seq.json'), 'w') as f:
