...
> 写于 2026-06-19，更新于 2026-06-23 (v3.0.23)。给接手的开发者。本文档目标是：任何人只看这一份文档就能理解全部代码、继续开发。
...
版本号:    v3.0.23 (versionCode 320) — 构建时动态注入 BuildConfig
```
...
kStomach    = 0.035 - isfFactor × 0.004   // 胃排空率 (★v3.0.22 中式慢排空)
VmaxGastric = 10.0  - isfFactor × 2.0     // 最大胃排空
Vm0         = 2.5   - isfFactor × 0.2     // 基础Uid (论文值, v3.0.20)
VmX         = 0.05  - isfFactor × 0.01    // 胰岛素乘数 (论文值, v3.0.20)
hepaticBase = 2.07  + isfFactor × 0.4     // 基础肝糖 (v3.0.20 稳态平衡)
kp3         = 0.045 - isfFactor × 0.007   // 胰岛素利用激活速率
```
...
|-----|-------------|-------------|-------------|------|
| kStomach | 0.030 | 0.035 | 0.042 | 中式慢排空 (v3.0.22) |
| VmaxGastric | 4.7 | 8.0 | 11.0 | 抵抗→胃排空上限低 |
| Vm0 | 2.0 | 2.5 | 2.8 | 论文值 (v3.0.20) |
| VmX | 0.04 | 0.05 | 0.07 | 论文值 (v3.0.20) |
| hepaticBase | 2.87 | 2.07 | 1.47 | 稳态平衡 (v3.0.20) |
| kp3 | 0.028 | 0.038 | 0.047 | 抵抗→作用慢 |
...
**增强 6: 速效胰岛素吸收加速 + 肠道吸收同步放缓 (v3.0.22)**

```kotlin
// 旧 → 新
ka1 = 0.018 → 0.024  // 非单体→单体加速, 峰值从~110min→~75min
ka2 = 0.018 → 0.024  // 单体→血浆加速
kGut = 0.065 → 0.050  // 肠道吸收同步放缓, 匹配慢胃排空
```

物理意义：速效胰岛素类似物（赖脯/门冬）的皮下吸收峰值在 60-90min，原参数 ka=0.018 导致峰值在 ~110min，偏慢。kGut 从 0.065 降到 0.050 配合 kStomach 的慢排空，维持胃-肠动力平衡。

**增强 7: 双相胃排空 (v3.0.23) — 修复"记录饮食后 DM 先降后升"**

```kotlin
// 进食 <15min: 20% 碳水直接入肠 (模拟液体/混合食物快速相)
//               80% 在胃慢相 (kStomach 驱动)
if (T < 15.0) {
    rapidGut = mg * 0.20 * exp(-kG * T)      // 快速入肠, 立即开始吸收
    slowStomach = mg * 0.80 * exp(-kS * T)    // 慢相仍在胃
    slowGut = mg * 0.80 * kS/(kG-kS) * (...)  // 慢相经胃→肠
} else {
    // >15min: 标准单相 (两相已融合)
}
```

物理意义：真实进食后，液体和部分小分子碳水在 10-15min 内快速通过幽门进入十二指肠（Meyer et al. 1985, Gastroenterology）。旧模型的单相胃排空导致刚吃完饭肠道仍是空的（Ra≈0），高血糖时 Uii 清糖快于碳水吸收 → 预测曲线先降后升。

修复效果（刚吃 80g @G=9.0, 空腹=7.0）：
- 修复前：9.0 → **8.8↓** (15min) → 9.6↑ → 10.4 — 先降后升 ❌
- 修复后：9.0 → **10.4↑** (15min) → 10.5 → 8.1 — 直接上升 ✅
...
#### 4.3.1 TCN 物理门控 R1-R7 (v3.0.22)

TCN 模型的 c 项（线性项）几乎恒正（ONNX 训练数据中下降样本不足），导致 TCN 在几乎所有场景下都预测血糖上升。通过 7 条物理规则检测 TCN 的不可信场景，触发后用 DallaMan 曲线主导融合：

```
R1: IOB > 1U && carb2h < 30g    → TCN 应降却升, 完全不可信
R2: IOB > 0.5 && TCN↑ && DM↓    → 方向冲突, DM 更可信
R3: carb2h ≥ 30 && TCN保守 && DM↓ → TCN 反应不足
R4: |TCN Δ30min| > 2.5           → 生理不可能 (暴涨/暴跌都门控)
R5: IOB > 1 && carb2h ≥ 30 && TCN↑ → 胰岛素主导, TCN 过激
R6: carb2h ≥ 30 && IOB < 0.5 && TCN > 2.0 → 仅碳水场景 TCN 过激
R7: TCN方向 × DM方向 < 0 && |TCN| ≥ 0.6 → 方向相反且偏差显著
```

**门控融合公式：**
```kotlin
if (physicalOverride) {
    // 75% DallaMan shape + 25% TCN 残差 (保留数据驱动贡献)
    alignedTcn[i] = G0 + 0.75 × (DM[i] - G0) + 0.25 × (TCN_raw[i] - G0)
    // ★ 陡降保护: DM 30min 跌 > 4.0 → DM 权重降至 50% (防过冲)
    if (|DM 30min| > 4.0) dmWeight = 0.50
}
```
...
```kotlin
// 门控触发时 TCN 权重进一步压低
tcnRatio = 0.2 - 0.15 × (|physioSlope30| / 0.5).coerceIn(0.0, 1.0)
tcnRatio = max(0.05, tcnRatio)  // 永远保留 5% TCN
```
